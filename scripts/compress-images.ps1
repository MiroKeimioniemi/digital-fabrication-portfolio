...
foreach ($ImageFile in $ImageFiles) {
    ...
    if ($ImageFile.Length -gt 100KB) {
        ...
        $ImageMagickCommand = "magick convert `"$ImageFile`" -strip -resize 1200x1200 -define webp:lossless=false -quality 75 -define webp:alpha-quality=80 -define webp:auto-filter=true -define webp:method=6 `"$OutputFile`""
